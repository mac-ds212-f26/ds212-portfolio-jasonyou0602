...
mn_cities <- st_read("data/unprocessed/City_And_Township_Population_Centers/City_And_Township_Population_Centers.shp")
mn_water <- st_read("data/unprocessed/Lakes_and_Rivers/Lakes_and_Rivers.shp")
```
...
st_crs(mn_cities) == st_crs(mn_water)
st_crs(mn_cities) == st_crs(mn_counties)
```
...
> The difference between _d(), _c() and _b() is the color scale.
...
ggsave("data/ica4.png", width = 5, height = 5, units = "in")
```
